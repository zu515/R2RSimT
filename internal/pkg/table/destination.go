...
func (lhs *PeerInfo) Equal(rhs *PeerInfo) bool {
	...
	if (lhs.AS == rhs.AS) && lhs.ID.Equal(rhs.ID) && lhs.LocalID.Equal(rhs.LocalID) && lhs.AddressAndPort == rhs.AddressAndPort {
		return true
	}
	return false
}
...
func (i *PeerInfo) String() string {
	...
	s.WriteString(fmt.Sprintf("{ %s | ", i.AddressAndPort))
...
func NewPeerInfo(g *config.Global, p *config.Neighbor) *PeerInfo {
	...
	nport := p.Transport.Config.RemotePort
	return &PeerInfo{
		AS:                      p.Config.PeerAs,
		LocalAS:                 g.Config.As,
		...
		Address:                 naddr.IP,
		AddressAndPort: 		net.JoinHostPort(naddr.IP.String(), strconv.Itoa(int(nport))),
