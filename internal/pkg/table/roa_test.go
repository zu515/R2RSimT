...
func TestValidate11(t *testing.T) {
	...
	table := NewROATable()
	table.Add(NewROA(bgp.AFI_IP, net.ParseIP("10.0.0.0").To4(), 8, 12, 666, ""))
	table.Add(NewROA(bgp.AFI_IP, net.ParseIP("10.0.0.0").To4(), 9, 12, 777, ""))


	r := validateOne(table, "10.0.0.0/9", "666")
	assert.Equal(r, config.RPKI_VALIDATION_RESULT_TYPE_VALID)
