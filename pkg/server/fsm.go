...
func (fsm *fsm) StateChange(nextState bgp.FSMState) {
	...
	log.WithFields(log.Fields{
		"Topic":  "Peer",
		"Key":    net.JoinHostPort(fsm.pConf.State.NeighborAddress, strconv.Itoa(int(fsm.pConf.Transport.Config.RemotePort))),
...
func (h *fsmHandler) connectLoop(ctx context.Context, wg *sync.WaitGroup) {
	...
	for {
		...
		case <-ctx.Done():
			log.WithFields(log.Fields{
				"Topic": "Peer",
				"Key":   net.JoinHostPort(addr, strconv.Itoa(int(port))),
		...
		if err == nil {
			...
			TopologyConfig.AddClientConPortMap(strings.Split(conn.LocalAddr().String(), ":")[0], strings.Split(conn.LocalAddr().String(), ":")[1], strconv.Itoa(int(TopologyConfig.NodeMap[fsm.gConf.Config.As].ServerPort)))
			//TopologyConfig.ClientConPortMap[conn.LocalAddr().String()] = net.JoinHostPort(TopologyConfig.NodeMap[fsm.gConf.Config.As].Adress, strconv.Itoa(int(TopologyConfig.NodeMap[fsm.gConf.Config.As].ServerPort)))
		...
		case conn, ok := <-fsm.connCh:
			...
			if ttl != 0 {
				if err := setTCPTTLSockopt(conn.(*net.TCPConn), ttl); err != nil {
					log.WithFields(log.Fields{
						"Topic": "Peer",
						"Key":   net.JoinHostPort(fsm.pConf.Config.NeighborAddress, strconv.Itoa(int(fsm.pConf.Transport.Config.RemotePort))),
						"State": fsm.state.String(),
...
func (h *fsmHandler) handlingError(m *bgp.BGPMessage, e error, useRevisedError bool) bgp.ErrorHandling {
	...
	if m.Header.Type == bgp.BGP_MSG_UPDATE && useRevisedError {
		...
		case bgp.ERROR_HANDLING_ATTRIBUTE_DISCARD:
			...
			log.WithFields(log.Fields{
				"Topic": "Peer",
				"Key":   net.JoinHostPort(h.fsm.pConf.State.NeighborAddress, strconv.Itoa(int(h.fsm.pConf.Transport.Config.RemotePort))),
...
func (h *fsmHandler) recvMessageWithError() (*fsmMsg, error) {
	...
	if err != nil {
		fmt.Println("错误 ", err, h.conn.LocalAddr(), h.conn.RemoteAddr(),h.conn)
	...
	if err != nil {
		...
		fmsg := &fsmMsg{
			...
			MsgSrc: net.JoinHostPort(h.fsm.pConf.Config.NeighborAddress, strconv.Itoa(int(h.fsm.pConf.Transport.Config.RemotePort))),
			MsgData: err,
		}
		h.fsm.lock.RUnlock()
		return fmsg, err
