...
func (s *BgpServer) passConnToPeer(conn *net.TCPConn) {
	...
	remoteAddrAndPort := net.JoinHostPort(host, port)

	//fmt.Println("内容1", s.neighborMap,s.bgpConfig.Global.Config.As)
	//fmt.Println("内容2", TopologyConfig.ClientConPortMap[remoteAddrAndPort])
	//fmt.Println("内容", TopologyConfig.ClientConPortMap)
	//fmt.Println("内容3", s.neighborMap[TopologyConfig.ClientConPortMap[remoteAddrAndPort]])

	peer, found := s.neighborMap[net.JoinHostPort(host, TopologyConfig.GetClientConPortMap(host, port))]

	//
	//
	//
	//fmt.Println("找到1", found)
	//fmt.Println("找到2", peer)
	//fmt.Println("找到3", TopologyConfig.GetClientConPortMap(host, port))
	//fmt.Println("找到4%+v", s.neighborMap)
	...
	if found {
		...
		log.WithFields(log.Fields{
			"Topic": "Peer",
		}).Debugf("Accepted a new passive connection from:%s", remoteAddrAndPort)
		peer.PassConn(conn)
		...
		s.neighborMap[remoteAddrAndPort] = peer
		...
		log.WithFields(log.Fields{
			"Topic": "Peer",
		}).Infof("Can't find configuration for a new passive connection from:%s", remoteAddrAndPort)
		conn.Close()
	}
}
...
func (s *BgpServer) addNeighbor(c *config.Neighbor) error {
	...
	addrAndPort ,err := c.ExtractNeighborAddressAndPort()
	if err != nil {
		return err
	}

	if _, y := s.neighborMap[addrAndPort]; y {
		return fmt.Errorf("can't overwrite the existing peer: %s", addrAndPort)
	}
	...
	log.WithFields(log.Fields{
		"Topic": "Peer",
	}).Infof("Add a peer configuration for:%s", addrAndPort)
	...
	s.neighborMap[addrAndPort] = peer
...
func (s *BgpServer) updateNeighbor(c *config.Neighbor) (needsSoftResetIn bool, err error) {
	...
	peer, ok := s.neighborMap[addrAndPort]
	if !ok {
		return needsSoftResetIn, fmt.Errorf("neighbor that has %v doesn't exist", addrAndPort)
	}
